...
const fetchData = () =>
	fetch(`https://api.nasa.gov/neo/rest/v1/feed?start_date=${getDate()}&api_key=DEMO_KEY`).then((res) => res.json());
...
export default function App() {
	...
	return (
		<div>
			<p>
				{format(addDays(new Date(), 1), 'EEEE d-MMM')} there will be <strong>{results.length}</strong> near
				misses
			</p>
			<hr />
			{results
				.sort((a) => (a.is_potentially_hazardous_asteroid ? -1 : 1))
				.map((data) => <Orbital key={data.id} {...data} />)}
		</div>
	);
}
